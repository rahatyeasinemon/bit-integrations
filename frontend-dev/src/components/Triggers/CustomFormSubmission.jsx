...
const CustomFormSubmission = () => {
  ...
  const setTriggerData = () => {
    ...
    tmpNewFlow.triggered_entity_id =
    ...
    setPrimaryKey((prev) =>
      create(prev, (draft) => {
        ...
        const primaryKey = keys.map((k) => ({
          key: k,
          value: newFlow.triggerDetail?.data?.find((item) => item.name === k)
            ?.value,
        }));
  ...
  return !newFlow?.triggerDetail?.is_active ? (
    <span className="mt-3">
      {newFlow?.triggerDetail?.name} is not installed or activated.
    </span>
  ) : (
    <div className="trigger-custom-width">
      ...
      {newFlow?.triggerDetail?.triggered_entity_id && (
        <>
          ...
          <div
            className={`flx mt-2 flx-${newFlow.triggerDetail?.data ? "between" : "around"}`}
          >
            <button
              onClick={handleFetch}
              ...
              <MultiSelect
                ...
                defaultValue={
                  Array.isArray(primaryKey)
                    ? primaryKey.map((item) => item.key).join(",")
                    : ""
                }
