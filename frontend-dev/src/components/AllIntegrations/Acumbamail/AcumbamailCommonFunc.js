...
export const checkAddressFieldMapRequired = (acumbamailConf) => {
  const requiredFleld = acumbamailConf?.address_field
    ? acumbamailConf.default?.fields.filter(
...
export const generateMappedField = (acumbamailConf) => {
  ...
  return requiredFlds.length > 0
    ? requiredFlds.map((field) => ({ formField: '', acumbamailFormField: field.key }))
    : [{ formField: '', acumbamailFormField: '' }]
}
