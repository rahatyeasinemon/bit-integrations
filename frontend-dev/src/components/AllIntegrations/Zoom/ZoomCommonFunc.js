    ...
    .then((result) => {
      if (result && result.success) {
        ...
        setSnackbar({
          ...
          msg: sprintf(
            __(
              "All Meeting list refresh failed Cause: %s. please try again",
              "bit-integrations"
            ),
        ...
        setSnackbar({
          show: true,
          msg: __(
            "All Meeting list failed. please try again",
            "bit-integrations"
          ),
        });
      }
...
export const checkMappedFields = (zoomConf) => {
  const mappedFleld = zoomConf.field_map
    ? zoomConf.field_map.filter(
        (mapped) => !mapped.formField && !mapped.zoomConf
      )
    : [];
