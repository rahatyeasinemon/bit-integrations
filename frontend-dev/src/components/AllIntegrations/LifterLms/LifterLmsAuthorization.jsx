  ...
  const authorizeHandler = () => {
    ...
    bitsFetch({}, 'lifterLms_authorize').then((result) => {
      if (result?.success) {
        ...
        setSnackbar({ show: true, msg: __('Connected with LifterLms Successfully', 'bit-integrations') })
      }
      setIsLoading(false)
      setShowAuthMsg(true)
    })
    ...
    >
      ...
      {showAuthMsg && !isAuthorized && !isLoading && (
        <div className="flx mt-5" style={{ color: 'red' }}>
          ...
          LifterLms plugin must be activated to integrate with Bit Integrations.
        </div>
      )}
