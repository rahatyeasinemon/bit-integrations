  ...
  return (
    <div className="btcd-stp-page" style={{ ...{ width: step === 1 && 900 }, ...{ height: step === 1 && 'auto' } }}>
      {zohoCreator?.youTubeLink && (
        <TutorialLink
          title={zohoCreator?.title}
          youTubeLink={zohoCreator?.youTubeLink}
        />
      )}
      {zohoCreator?.docLink && (
        <TutorialLink
          title={zohoCreator?.title}
          docLink={zohoCreator?.docLink}
        />
      )}
      ...
      {!isInfo && (
        <>
          <button onClick={() => handleAuthorize(creatorConf, setCreatorConf, setError, setisAuthorized, setIsLoading, setSnackbar)} className="btn btcd-btn-lg purple sh-sm flx" type="button" disabled={isAuthorized || isLoading}>
