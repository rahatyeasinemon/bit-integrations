...
export default function ZohoAuthorization({ integ, tutorialLink, scopes, nextPage, config, setConfig, step, isLoading, setIsLoading, setSnackbar, redirectLocation, isInfo }) {
    ...
    return (
        <div className="btcd-stp-page" style={{ ...{ width: step === 1 && 900 }, ...{ height: step === 1 && 'auto' } }}>
            ...
            <input className="btcd-paper-inp w-6 mt-1" onChange={handleInput} name="name" value={config.name} type="text" placeholder={__('Integration Name...', 'bit-integrations')} disabled={isInfo} />

            <div className="mt-3"><b>{__('Data Center:', 'bit-integrations')}</b></div>
            <select onChange={handleInput} name="dataCenter" value={config.dataCenter} className="btcd-paper-inp w-6 mt-1" disabled={isInfo}>
            ...
            <small className="d-blk mt-5">
                ...
                {' '}
                <a className="btcd-link" href={`https://api-console.${config?.dataCenter || 'zoho.com'}/`} target="_blank" rel="noreferrer">{__('Zoho API Console', 'bit-integrations')}</a>
            </small>

            <div className="mt-3"><b>{__('Client id:', 'bit-integrations')}</b></div>
            <input className="btcd-paper-inp w-6 mt-1" onChange={handleInput} name="clientId" value={config.clientId} type="text" placeholder={__('Client id...', 'bit-integrations')} disabled={isInfo} />
            <div style={{ color: 'red' }}>{error.clientId}</div>

            <div className="mt-3"><b>{__('Client secret:', 'bit-integrations')}</b></div>
            <input className="btcd-paper-inp w-6 mt-1" onChange={handleInput} name="clientSecret" value={config.clientSecret} type="text" placeholder={__('Client secret...', 'bit-integrations')} disabled={isInfo} />
            <div style={{ color: 'red' }}>{error.clientSecret}</div>

            {!isInfo && (
                <>
                    <button onClick={() => handleAuthorize(integ, scopes, config, setConfig, setError, setisAuthorized, setIsLoading, setSnackbar, btcbi)} className="btn btcd-btn-lg green sh-sm flx" type="button" disabled={isAuthorized || isLoading}>
