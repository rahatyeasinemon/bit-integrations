...
export default function GiveWpAuthorization({ formID,
  giveWpConf,
  setGiveWpConf,
  step,
  setStep,
  ...
  const authorizeHandler = () => {
    ...
    bitsFetch({}, 'giveWp_authorize').then((result) => {
      if (result?.success) {
        ...
        setSnackbar({ show: true, msg: __('Connected with GiveWp Successfully', 'bit-integrations') })
      }
      setIsLoading(false)
      setShowAuthMsg(true)
    })
  ...
  return (
    ...
    >
      {giveWp?.youTubeLink && (
        <TutorialLink
          title={giveWp?.title}
          youTubeLink={giveWp?.youTubeLink}
        />
      )}
      {giveWp?.docLink && (
        <TutorialLink
          title={giveWp?.title}
          docLink={giveWp?.docLink}
        />
      )}
      ...
      {showAuthMsg && !isAuthorized && !isLoading && (
        <div className="flx mt-5" style={{ color: 'red' }}>
          ...
          GiveWp affiliate plugin must be activated to integrate with Bit Integrations.
        </div>
      )}
