...
export default function WooCommerceAuthorization({ formID, wcConf, setWcConf, step, setStep, setSnackbar }) {
  ...
  const authorizeHandler = () => {
    setIsLoading('auth')
    bitsFetch({}, 'wc_authorize')
      .then(result => {
        if (result?.success) {
          setisAuthorized(true)
          setSnackbar({ show: true, msg: __('Connected with WooCommerce Successfully', 'bit-integrations') })
        }
        setIsLoading(false)
        setShowAuthMsg(true)
      })
  ...
  return (
    ...
    >
      ...
      <input className="btcd-paper-inp w-6 mt-1" onChange={handleInput} name="name" value={wcConf.name} type="text" placeholder={__('Integration Name...', 'bit-integrations')} />
      ...
      {(showAuthMsg && !isAuthorized && !isLoading) && (
        <div className="flx mt-5" style={{ color: 'red' }}>
          ...
          WooCommerce plugin must be activated to integrate with Bit Integrations.
        </div>
      )}
