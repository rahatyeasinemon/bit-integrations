...
export default function GetgistAuthorization({ getgistConf, setGetgistConf, step, setstep, isInfo }) {
  ...
  const handleAuthorize = () => {
    ...
    bitsFetch(data, 'getgist_authorize')
      .then(result => {
        if (result?.success) {
  ...
  return (
    <div className="btcd-stp-page" style={{ ...{ width: step === 1 && 900 }, ...{ height: step === 1 && 'auto' } }}>
      ...
      <input className="btcd-paper-inp w-6 mt-1" onChange={handleInput} name="api_key" value={getgistConf.api_key} type="text" placeholder={__('Integration Name...', 'bit-integrations')} disabled={isInfo} />
      <div style={{ color: 'red', fontSize: '15px' }}>{error.api_key}</div>
      <small className="d-blk mt-5">
        {__('To get API , Please Visit', 'bit-integrations')}
        {' '}
        <a className="btcd-link" href="https://app.getgist.com/projects/jgmmrszy/settings/api-key" target="_blank" rel="noreferrer">{__('Getgist API Console', 'bit-integrations')}</a>
      </small>
      {isLoading === 'auth' && (
