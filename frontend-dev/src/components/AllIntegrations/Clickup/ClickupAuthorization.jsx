  ...
  return (
    <div className="btcd-stp-page" style={{ ...{ width: step === 1 && 900 }, ...{ height: step === 1 && 'auto' } }}>
      {clickup?.youTubeLink && (
        <TutorialLink
          title={clickup?.title}
          youTubeLink={clickup?.youTubeLink}
        />
      )}
      {clickup?.docLink && (
        <TutorialLink
          title={clickup?.title}
          docLink={clickup?.docLink}
        />
      )}
      ...
      {!isInfo && (
        <div>
          <button onClick={() => clickupAuthentication(clickupConf, setClickupConf, setError, setIsAuthorized, loading, setLoading)} className="btn btcd-btn-lg purple sh-sm flx" type="button" disabled={isAuthorized || loading.auth}>
