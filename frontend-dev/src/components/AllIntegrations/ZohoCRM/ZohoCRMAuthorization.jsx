  ...
  return (
    <div className="btcd-stp-page" style={{ ...{ width: step === 1 && 900 }, ...{ height: step === 1 && 'auto' } }}>
      {zohoCRM?.youTubeLink && (
        <TutorialLink
          title={zohoCRM?.title}
          youTubeLink={zohoCRM?.youTubeLink}
        />
      )}
      {zohoCRM?.docLink && (
        <TutorialLink
          title={zohoCRM?.title}
          docLink={zohoCRM?.docLink}
        />
      )}
      ...
      <small className="d-blk mt-5">
        ...
        {' '}
        <a className="btcd-link" href={`https://api-console.zoho.${crmConf?.dataCenter || 'com'}/`} target="_blank" rel="noreferrer">{__('Zoho API Console', 'bit-integrations')}</a>
      </small>
      ...
      {!isInfo && (
        <>
          <button onClick={() => handleAuthorize('zohoCRM', 'zcrm', scopes, crmConf, setCrmConf, setError, setisAuthorized, setIsLoading, setSnackbar, btcbi)} className="btn btcd-btn-lg purple sh-sm flx" type="button" disabled={isAuthorized || isLoading}>
