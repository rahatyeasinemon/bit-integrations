    ...
    <div className="btcd-stp-page" style={{ ...{ width: step === 1 && 900 }, ...{ height: step === 1 && `${100}%` } }}>
      {zoomWebinar?.youTubeLink && (
        <TutorialLink
          ...
          youTubeLink={zoomWebinar?.youTubeLink}
        />
      )}
      {zoomWebinar?.docLink && (
        <TutorialLink
          title={zoomWebinar?.title}
          docLink={zoomWebinar?.docLink}
        />
      )}
      ...
      {!isInfo && (
        <>
          <button onClick={() => handleAuthorize(zoomWebinarConf, setZoomWebinarConf, setError, setisAuthorized, setIsLoading, setSnackbar)} className="btn btcd-btn-lg purple sh-sm flx" type="button" disabled={isAuthorized}>
