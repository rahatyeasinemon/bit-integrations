...
export default function BenchMarkFieldMap({ i, formFields, field, benchMarkConf, setBenchMarkConf }) {
  ...
  const notResquiredField = benchMarkConf?.default?.fields && Object.values(benchMarkConf?.default?.fields).filter((f => !f.required))
  ...
  return (
    <div className="flx mt-2 mb-2 btcbi-field-map">
      <div className="flx integ-fld-wrp">
        ...
        <select className="btcd-paper-inp" name="benchMarkField" value={field.benchMarkField} onChange={(ev) => handleFieldMapping(ev, i)} disabled={isRequired}>
          <option value="">{__('Select Field', 'bit-integrations')}</option>
          {isRequired ? benchMarkConf?.default?.fields && Object.values(benchMarkConf.default.fields).map(fld => (
