...
export default function DirectIqFieldMap({ i, formFields, field, directIqConf, setDirectIqConf }) {
  ...
  const notResquiredField = directIqConf?.default?.fields && Object.values(directIqConf?.default?.fields).filter((f => !f.required))
  ...
  return (
    <div className="flx mt-2 mb-2 btcbi-field-map">
      <div className="flx integ-fld-wrp">
        ...
        <select className="btcd-paper-inp" name="directIqField" value={field.directIqField} onChange={(ev) => handleFieldMapping(ev, i)} disabled={isRequired}>
          <option value="">{__('Select Field', 'bit-integrations')}</option>
          {isRequired ? directIqConf?.default?.fields && Object.values(directIqConf.default.fields).map(fld => (
