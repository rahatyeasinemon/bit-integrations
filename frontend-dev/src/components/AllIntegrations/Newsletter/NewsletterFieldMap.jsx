    ...
    >
      <div className="pos-rel flx">
        <div className="flx integ-fld-wrp">
          <select className="btcd-paper-inp mr-2" name="formField" value={field.formField || ''} onChange={(ev) => handleFieldMapping(ev, i, newsletterConf, setNewsletterConf)}>
          ...
          {field.formField === 'custom' && <TagifyInput onChange={e => handleCustomValue(e, i, newsletterConf, setNewsletterConf)} label={__('Custom Value', 'bit-integrations')} className="mr-2" type="text" value={field.customValue} placeholder={__('Custom Value', 'bit-integrations')} formFields={formFields} />}

          <select className="btcd-paper-inp" disabled={i < requiredFields.length} name="newsletterFormField" value={i < requiredFields ? (requiredFields[i].label || '') : (field.newsletterFormField || '')} onChange={(ev) => handleFieldMapping(ev, i, newsletterConf, setNewsletterConf)}>
            <option value="">{__('Select Field', 'bit-integrations')}</option>
            {
          ...
          }
          {
            i >= requiredFields.length && (
              <>
                ...
                <button onClick={() => delFieldMap(i, newsletterConf, setNewsletterConf)} className="icn-btn sh-sm ml-1" type="button" aria-label="btn">
