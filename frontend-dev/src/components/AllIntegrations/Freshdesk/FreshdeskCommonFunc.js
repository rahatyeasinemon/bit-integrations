...
export const getAllTicketFields = (
  confTmp,
  setConf,
  setIsLoading,
  setSnackbar
    ...
    .then((result) => {
      if (result && result.success) {
        ...
        setSnackbar({
          show: true,
          msg: __(
            "Ticket field fetch failed. please try again",
            "bit-integrations"
          ),
        });
      }
...
export const getAllContactFields = (
  confTmp,
  setConf,
  setIsLoading,
  setSnackbar
    ...
    .then((result) => {
      if (result && result.success) {
        ...
        setSnackbar({
          show: true,
          msg: __(
            "Contacts field fetch failed. please try again",
            "bit-integrations"
          ),
        });
      }
...
export const checkMappedFields = (fieldsMapped) => {
  const checkedField = fieldsMapped
    ? fieldsMapped?.filter(
        (item) => !item.formField || !item.freshdeskFormField
      )
    : [];
...
export const checkMappedFieldsContact = (fieldsMapped) => {
  const checkedField = fieldsMapped
    ? fieldsMapped?.filter(
        (item) => !item.formField || !item.contactFreshdeskFormField
      )
    : [];
  if (checkedField.length > 0) return false;
  return true;
...
export const generateMappedField = (freshdeskConf) => {
  const requiredFlds = freshdeskConf?.ticketFields.filter(
...
export const generateContactMappedField = (freshdeskConf) => {
  const requiredFlds = freshdeskConf?.contactFields.filter(
    (fld) => fld.required === true
  );
  return requiredFlds.length > 0
    ? requiredFlds.map((field) => ({
