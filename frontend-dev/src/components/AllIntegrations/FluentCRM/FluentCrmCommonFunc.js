  ...
  bitsFetch({}, 'refresh_fluent_crm_lists')
    .then(result => {
      if (result && result.success) {
        ...
        setSnackbar({ show: true, msg: __('FluentCRM list refreshed', 'bit-integrations') })
        refreshfluentCrmHeader(newConf, setFluentCrmConf, setIsLoading, setSnackbar)
      } else if ((result && result.data && result.data.data) || (!result.success && typeof result.data === 'string')) {
        setSnackbar({ show: true, msg: `${__('FluentCRM list refresh failed Cause:', 'bit-integrations')}${result.data.data || result.data}. ${__('please try again', 'bit-integrations')}` })
      } else {
        setSnackbar({ show: true, msg: __('FluentCRM list refresh failed. please try again', 'bit-integrations') })
      }
      setIsLoading(false)
    })
...
export const refreshCrmTag = (formID, fluentCrmConf, setFluentCrmConf, setIsLoading, setSnackbar) => {
  setIsLoading(true)
  bitsFetch({}, 'refresh_fluent_crm_tags')
    .then(result => {
      if (result && result.success) {
        ...
        setSnackbar({ show: true, msg: __('FluentCRM Tags refreshed', 'bit-integrations') })
        refreshfluentCrmHeader(newConf, setFluentCrmConf, setIsLoading, setSnackbar)
      } else if ((result && result.data && result.data.data) || (!result.success && typeof result.data === 'string')) {
        setSnackbar({ show: true, msg: `${__('FluentCRM tags refresh failed Cause:', 'bit-integrations')}${result.data.data || result.data}. ${__('please try again', 'bit-integrations')}` })
      } else {
        setSnackbar({ show: true, msg: __('FluentCRM tags refresh failed. please try again', 'bit-integrations') })
      }
      setIsLoading(false)
    })
    .catch(() => setIsLoading(false))
    ...
    .then(result => {
      if (result && result.success) {
        ...
        if (result.data.fluentCrmFlelds) {
          ...
          setSnackbar({ show: true, msg: __('Fluent CRM fields refreshed', 'bit-integrations') })
        } else {
          setSnackbar({ show: true, msg: __('No Fluent CRM fields found. Try changing the header row number or try again', 'bit-integrations') })
        }
        setFluentCrmConf({ ...newConf })
      } else {
        setSnackbar({ show: true, msg: __('Fluent CRM fields refresh failed. please try again', 'bit-integrations') })
      }
      setIsLoading(false)
    })
    .catch(() => setIsLoading(false))
