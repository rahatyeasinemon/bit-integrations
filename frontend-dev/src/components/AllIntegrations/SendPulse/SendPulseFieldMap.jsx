...
export default function SendPulseFieldMap({ i, formFields, field, sendPulseConf, setSendPulseConf }) {
  ...
  const notResquiredField = sendPulseConf?.default?.fields && Object.values(sendPulseConf?.default?.fields).filter((f => !f.required))
  ...
  return (
    <div className="flx mt-2 mb-2 btcbi-field-map">
      <div className="flx integ-fld-wrp">
        ...
        <select className="btcd-paper-inp" name="sendPulseField" value={field.sendPulseField || ''} onChange={(ev) => handleFieldMapping(ev, i)} disabled={isRequired}>
          <option value="">{__('Select Field', 'bit-integrations')}</option>
          {isRequired ? sendPulseConf?.default?.fields && Object.values(sendPulseConf.default.fields).map(fld => (
