...
export default function TreeViewer({ data = [], draggable = false }) {
    const formatedData = useMemo(() => [processData(data, 0, textIcon)], [data])
    console.log(JSON.stringify(formatedData))

    // Define the functions for handling tree list changes
...
const processData = (data, index, textIcon) => {
    // Handle non-object (including null) and non-array data
    if (typeof data !== "object" || data === null) {
        return { label: index, icon: textIcon };
    }
