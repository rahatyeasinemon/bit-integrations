...
export default function TableCheckBox(props) {
  ...
  return (
    <label htmlFor={`btcd-cbx-${id}`} className={`btcd-label-cbx ${props.className}`}>
      <input id={`btcd-cbx-${id}`} type="checkbox" checked={props.checked} value={props.value} name={props.name} onChange={props.onChange} className="btcd-cbx-invisible" {...props.rest} ref={props.refer} disabled={props.isInfo} />
