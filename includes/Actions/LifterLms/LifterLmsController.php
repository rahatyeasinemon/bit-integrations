...
class LifterLmsController
{
    ...
    public static function getAllLifterLmsCourse()
    {
        ...
        $allCourse = $wpdb->get_results($wpdb->prepare("SELECT ID, post_title FROM %s
        WHERE %s = 'publish' AND %s = 'course' ORDER BY post_title", $wpdb->posts, "$wpdb->posts.post_status", "$wpdb->posts.post_type"));
    ...
    public static function getAllLifterLmsMembership()
    {
        ...
        $allMembership = $wpdb->get_results($wpdb->prepare("SELECT ID, post_title FROM %s
        WHERE %s = 'publish' AND %s = 'llms_membership' ORDER BY post_title", $wpdb->posts, "$wpdb->posts.post_status", "$wpdb->posts.post_type"));
